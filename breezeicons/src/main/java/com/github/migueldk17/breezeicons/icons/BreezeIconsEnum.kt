package com.github.migueldk17.breezeicons.icons

/**
 * Enum class that lists all available icons in the BreezeIcons library.
 *
 * Each enum constant represents a unique icon and is used for logical identification and type safety
 * when working with BreezeIconsType.
 *
 * Use this enum to refer to specific icons programmatically, for example when setting an icon in the UI.
 */
enum class BreezeIconsEnum {

    AIRPLANE_LINEAR,
    AIRPODS_LINEAR,
    ALT_ARROW_DOWN,
    ALT_ARROW_LEFT,
    ALT_ARROW_RIGHT,
    ALT_ARROW_UP,
    ANDROID_LINEAR,
    BAG2_LINEAR,
    BAG_OUTLINED,
    BOOK_LINEAR,
    BOOK_OUTLINED,
    BUS_LINEAR,
    CALENDAR_LINEAR,
    CALENDAR_OUTLINED,
    CAR_LINEAR,
    CASE_ROUND_MINIMALISTIC_LINEAR,
    CHAT_SQUARE_LINEAR,
    CHEF_HAT_LINEAR,
    CHRISTMAS_TREE,
    CLOUD_LINEAR,
    CLOUD_OUTLINED,
    CONFETTI_MINIMALISTIC_LINEAR,
    DISCOVER_LINEAR,
    DISCOVERY_OUTLINED,
    DOCUMENT_LINEAR,
    DOLLAR_CIRCLE_LINEAR,
    DOLLAR_CIRCLE_OUTLINED,
    DOLLAR_SQUARE_LINEAR,
    DOLLAR_SQUARE_OUTLINED,
    DONUT,
    DROP_LINEAR,
    DROP_OUTLINED,
    EDIT_LINEAR,
    FACEBOOK_LINEAR,
    FILE_TEXT_LINEAR,
    FORWARD_LINEAR,
    FORWARD_OUTLINED,
    FORWARD_PLAYER_LINEAR,
    GAME_LINEAR,
    GAMEPAD_LINEAR,
    GAS_STATION_LINEAR,
    GLOBE_LINEAR,
    GLOBE_OUTLINED,
    GOOGLE_LINEAR,
    GOOGLE_PLAY_LINEAR,
    GROUP_LINEAR,
    GROUP_OUTLINED,
    HANGER_LINEAR,
    HEADPHONES_ROUND_LINEAR,
    HEART_LINEAR,
    HOME_LINEAR,
    HOSPITAL_LINEAR,
    HOSPITAL_OUTLINED,
    HOUSE_LINEAR,
    ICON_BLUE_ROYAL_VIBRANT,
    ICON_BLUE_VIBRANT,
    ICON_GREEN_CYAN_VIBRANT,
    ICON_GREEN_VIBRANT,
    ICON_MAGENTA_VIBRANT,
    ICON_ORANGE_VIBRANT,
    ICON_PINK_VIBRANT,
    ICON_PURPLE_VIBRANT,
    ICON_RED_VIBRANT,
    ICON_SOFT_BLUE,
    ICON_SOFT_CYAN,
    ICON_SOFT_GREEN,
    ICON_SOFT_GREEN_CYAN,
    ICON_SOFT_MAGENTA,
    ICON_SOFT_ORANGE,
    ICON_SOFT_PINK,
    ICON_SOFT_PURPLE,
    ICON_SOFT_RED,
    ICON_SOFT_ROYAL_BLUE,
    ICON_SOFT_YELLOW,
    ICON_TURQUOISE_VIBRANT,
    ICON_UNSPECIFIED,
    ICON_YELLOW_VIBRANT,
    INFO_CIRCLE_LINEAR,
    INFO_CIRCLE_OUTLINED,
    KEY_LINEAR,
    KEY_OUTLINED,
    LAPTOP_LINEAR,
    LIKE_LINEAR,
    LIKE_OUTLINED,
    MAGIC_PEN_LINEAR,
    MAGIC_PEN_OUTLINED,
    MENU_DOTS,
    MONEY_RECEIVE_LINEAR,
    MONEY_RECEIVE_OUTLINED,
    MONEY_SEND_LINEAR,
    MONEY_SEND_OUTLINED,
    NOTIFICATION_BING_LINEAR,
    NOTIFICATION_BING_OUTLINED,
    NOTIFICATION_LINEAR,
    NOTIFICATION_OUTLINED,
    PAPERBIN_LINEAR,
    PAPERBIN_OUTLINED,
    PAW,
    PLAY_CIRCLE_OUTLINED,
    RECYCLE_LINEAR,
    REFRESH_LINEAR,
    RESTART_LINEAR,
    SETTINGS_LINEAR,
    SETTINGS_OUTLINED,
    SKIRT_LINEAR,
    SLEEPING_LINEAR,
    SPOTIFY_LINEAR,
    SQUARE_ACADEMIC_CAP_2_LINEAR,
    T_SHIRT_LINEAR,
    TEA_CUP_LINEAR,
    TICKER_STAR_LINEAR,
    TRAFFIC_ECONOMY,
    UMBRELLA_LINEAR,
    VERIFIED_CHECK,
    VIDEO_CIRCLE_LINEAR,
    WHATSAPP_LINEAR,
    BANK_LINEAR,
    BOX_LINEAR,
    CALL_LINEAR,
    CLOCK_LINEAR,
    CLOUD_ADD_LINEAR,
    CLOUD_CROSS_LINEAR,
    CLOUD_DRIZZLE_LINEAR,
    CLOUD_LIGHTNING_LINEAR,
    CLOUD_MINUS_LINEAR,
    CLOUD_NOTIF_LINEAR,
    CLOUD_PLUS_LINEAR,
    CLOUD_REMOVE_LINEAR,
    CLOUD_SNOW_LINEAR,
    CLOUD_SUNNY_LINEAR,
    CLOUD_WEATHER_LINEAR,
    EYE_LINEAR,
    EYE_SLASH_LINEAR,
    HOME_TREND_DOWN_LINEAR,
    HOME_TREND_UP_LINEAR,
    LOCK_LINEAR,
    MICROPHONE_LINEAR,
    MOON_LINEAR,
    PROFILE_2USER_LINEAR,
    PROFILE_LINEAR,
    SEARCH_NORMAL_LINEAR,
    SECURITY_LINEAR,
    SHOPPING_CART_LINEAR,
    SLASH_LINEAR,
    SUN_LINEAR,
    TREND_DOWN_LINEAR,
    TREND_UP_LINEAR,
    USER_LINEAR,
    WINDOWS_LINEAR,

    EXPORT_LINEAR,
    EXPORT_OUTLINED,
    HEART_OUTLINED,
    LOGOUT_LINEAR,
    LOGOUT_OUTLINED,
    REPEAT_LINEAR,
    REPEAT_OUTLINED,
    STAR_OUTLINED,
    CHAT_ROUND_LINEAR,
    CHAT_ROUND_OUTLINED,
    PEN_LINEAR,
    PEN_OUTLINED

}
